...
public class ADExecutor extends ThreadPoolExecutor{
    private static final int DEFAULT_THREAD_COUNT = 3;

    public ADExecutor(int maxNumThreads, ThreadFactory threadFactory) {
    ...
    private static final class ADownloadFutureTask extends FutureTask<CoreRunnable> implements Comparable<ADownloadFutureTask> {
        private final CoreRunnable hunter;

        public ADownloadFutureTask(CoreRunnable hunter) {
            super(hunter, null);
            this.hunter = hunter;
        }

        @Override
        public int compareTo(ADownloadFutureTask other) {
